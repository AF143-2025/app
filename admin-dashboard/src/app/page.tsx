...
import { adminApiFetch, getAdminToken, removeAdminToken, API_BASE_URL } from "@/lib/api";
...
export default function StandaloneAdminDashboard() {
  ...
  const loadDeals = async () => {
    try {
      ...
      const res = await adminApiFetch("/api/admin/deals");
      const data = await res.json();
      if (data.success) {
        setDeals(data.deals || []);
      }
  ...
  useEffect(() => {
    if (activeTab === "products" && getAdminToken()) {
  ...
  const handleLogout = async () => {
    try {
      await adminApiFetch("/api/admin/auth/logout", { method: "POST" });
    } catch (e) {
      // ignore
    } finally {
      removeAdminToken();
  ...
  const handleToggleProductActive = async (p: any) => {
    try {
      const res = await adminApiFetch(`/api/admin/products/${p.id}`, {
        method: "PUT",
  ...
  const handleDeleteProduct = (p: any) => {
    setDeleteDialog({
      ...
      onConfirm: async () => {
        try {
          const res = await adminApiFetch(`/api/admin/products/${p.id}`, { method: "DELETE" });
          const data = await res.json();
          if (data.success) {
  ...
  const handleDeleteCategory = (c: any) => {
    setDeleteDialog({
      ...
      onConfirm: async () => {
        try {
          const res = await adminApiFetch(`/api/admin/categories/${c.id}`, { method: "DELETE" });
          const data = await res.json();
          if (data.success) {
  ...
  const handleRemoveDeal = (productId: string, productName: string) => {
    setDeleteDialog({
      ...
      onConfirm: async () => {
        try {
          const res = await adminApiFetch(`/api/admin/deals?productId=${encodeURIComponent(productId)}`, {
            method: "DELETE",
          });
          const data = await res.json();
          if (data.success) {
  ...
  const handleDeleteBanner = (b: any) => {
    setDeleteDialog({
      ...
      onConfirm: async () => {
        try {
          const res = await adminApiFetch(`/api/admin/banners/${b.id}`, { method: "DELETE" });
          const data = await res.json();
          if (data.success) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await adminApiFetch("/api/admin/auth/password", {
        method: "POST",
        body: JSON.stringify({
